...
context "Command connections" do
  specify "should build correct sequel connection string from activerecord config hash" do
    Tentacle::Command.yaml_to_connection_string(:adapter => 'mysql', :username => 'rick', :password => 'secret', :host => 'myhost', :database => 'test').should == \
  ...
  specify "should build correct sequel connection string from activerecord config hash without host value" do
    Tentacle::Command.yaml_to_connection_string(:adapter => 'mysql', :username => 'rick', :password => 'secret', :database => 'test').should == \
  ...
  specify "should build correct sequel connection string from activerecord config hash using postgresql" do
    Tentacle::Command.yaml_to_connection_string(:adapter => 'postgresql', :username => 'rick', :password => 'secret', :host => 'myhost', :database => 'test').should == \
  ...
  specify "should build correct sequel connection string from activerecord config hash using sqlite3" do
    Tentacle::Command.yaml_to_connection_string(:adapter => 'sqlite3', :username => 'rick', :password => 'secret', :host => 'myhost', :database => 'test').should == \
      "sqlite://test"
  end

  specify "should warn against sqlite2 usage" do
    assert_raises RuntimeError do
      Tentacle::Command.yaml_to_connection_string(:adapter => 'sqlite', :database => 'test')
    end
  end
end