module Tentacle
  ...
  def self.session_options
    default_session_options.merge :session_domain => ".#{Tentacle.domain}", :session_path => '/'
  end
  ...
  class << self
    ...
    def setup!(&block)
      ...
      if Object.const_defined?(:USE_REPO_PATHS) && USE_REPO_PATHS
        puts "** Using paths for repositories, instead of subdomains.  http://#{Tentacle.domain || 'my-domain.com'}/my-repo/browser, etc."
      end
    end
    ...
    def setup_mail!
      ...
      elsif Tentacle.const_defined?(:Mailer)
        Tentacle::Mailer.delivery_method = :test_send
        return if RAILS_ENV == 'test' || mail_type.nil? || send("#{mail_type}_settings").nil? || send("#{mail_type}_settings").empty?
        options   = send("#{mail_type}_settings")
        Tentacle::Mailer.delivery_method = mail_type == 'smtp' ? :net_smtp : :sendmail
        Tentacle::Mailer.config = {}
        return if Tentacle::Mailer.delivery_method == :sendmail
        Tentacle::Mailer.config[:domain] = options[:domain]         if options[:domain] && options[:domain].size > 0
        Tentacle::Mailer.config[:host]   = options[:address]        if options[:address] && options[:address].size > 0
        Tentacle::Mailer.config[:port]   = options[:port]           if options[:port]
        Tentacle::Mailer.config[:user]   = options[:user_name]      if options[:user_name] && options[:user_name].size > 0
        Tentacle::Mailer.config[:pass]   = options[:password]       if options[:password] && options[:password].size > 0
        Tentacle::Mailer.config[:auth]   = options[:authentication] if options[:authentication] && options[:authentication].size > 0
      end
    end
    ...
    def write_config_file(attributes = {})
      ...
      tmpl = ['# This file is auto generated.  Visit /admin/settings to change it.', '#', "require 'tentacle' unless Object.const_defined?(:Tentacle)", '', 'Tentacle.setup! do']
      ...
      ApplicationController.session(Tentacle.session_options) if domain_is_blank && !attributes[:domain].blank?
    end
  end

  self.config_path             ||= File.join(RAILS_ROOT, 'config', 'initializers', 'tentacle.rb')
  self.default_session_options ||= {:session_key => '_tentacle_session_id', :secret => '4b3eaf64bfa62da140e0f45c9030f272'}
  self.domain                  ||= ''
...
unless !File.exist?(File.join(RAILS_ROOT, 'config/initializers/tentacle'))
  require 'config/initializers/tentacle'
end

if Object.const_defined?(:Dependencies)
  Dependencies.autoloaded_constants.delete 'Tentacle'
end