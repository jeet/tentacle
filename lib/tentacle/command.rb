...
module Tentacle
  class Command
    ...
    def sync_revisions_for(repo_subdomain, num = 0)
      ...
      Tentacle::Syncer.const_get(repo[:scm_type].capitalize + "Syncer").process(connection, repo, silo_for(repo), num)
    end
    
    def process_hooks_for(repo_subdomain, repo_path, revision)
      repo         = find_repo(repo_subdomain)
      hook_options = indexed_hooks(hooks_for(repo))
      Tentacle::Hooks::Commit.run repo, repo_path, revision, hook_options
    end
  ...
  protected
    ...
    def indexed_hooks(hooks)
      hooks.inject [] do |memo, hook|
        memo << [Tentacle::Hooks[hook[:name]], YAML.load(hook[:options])]
      end
    end
