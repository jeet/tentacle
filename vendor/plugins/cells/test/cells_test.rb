...
class CellTestController < ApplicationController
  ...
  def render_view_with_render_cell_invocation
    render :template => "../../vendor/plugins/zells/test/views/view_with_render_cell_invocation"
    return
  end
  ...
  def cells_render_invocation
    render :template => "../../vendor/plugins/zells/test/views/view_with_cells_render_invocation"
    return
  end
...
class TestCell < Cell::Base
  ...
  def view_for_state(state)
    RAILS_ROOT+"/vendor/plugins/zells/test/views/#{state}.rhtml"
  end
...
class CellsTest < Test::Unit::TestCase
  ...
  def self.path_to_test_views
    RAILS_ROOT + "/vendor/plugins/zells/test/views/"
  end
  ...
  def setup
    ...
    # this may break too.
    Rails.plugins[:zells].code_paths |= Rails.plugins[:zells].default_code_paths
    Rails.plugins[:zells].inject_into_load_path
...
#    assert_tag :tag => "span", :child => /^yeah$/
  ...
  def test_render
    ...
    assert_equal cell.render_state("direct_output").class, String
    #assert_equal cell.render_state("rendering_state").class, String
    #puts cell.render_state("rendering_state")
  ...
  def test_view_for_state_overwriting
    ...
    #
    #puts cell.render_state(:some_state)
  ...
  #require RAILS_ROOT + "/vendor/plugins/zells/app/cells/cells_test_one_cell.rb"
  ...
  def test_class_autoloading
    ...
    # puts Plugin.default_code_paths  ### protected method.

    #puts Dependencies.load_paths
    puts "CELLS PATHS:"
    puts  Dependencies.load_paths.select{|x| x =~ /cells/ }
  ...
  def test_helper_including_and_cleanup
    ...
    #puts cell.render_state(:state_with_not_included_helper_method)
    puts "state with NOT loaded helper:"
