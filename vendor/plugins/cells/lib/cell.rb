module Cell
  ...
  class Base
    ...
    def render_view_for_state(state)
      ...
      include_helpers_in_class(self.class.helper_modules, view_class)
      clone_ivars_to(action_view)
      action_view.render_template(File.extname(view_file)[1..-1], nil, view_file)
    end
    ...
    def include_helpers_in_class(helper_modules, klass)
      self.class.helper_modules.each do |helper_module|
        klass.send :include, helper_module
      end
    end
    ...
    # Define a helper module name to include in this cell's state views.
    def self.helper(helper_module)
    #def self.helper(*args)
      @helper_modules ||= [ApplicationHelper]
      @helper_modules << helper_module
    end

    # Getter for the current cell's helper modules
    def self.helper_modules
      # that's an instance variable of the current class. isn't it cool?
      @helper_modules ||= [ApplicationHelper]
    end
  end
end
