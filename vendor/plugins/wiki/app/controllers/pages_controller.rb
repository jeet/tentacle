class PagesController < ApplicationController
  ...
  before_filter :admin_required, :only => [:lock]
  ...
  # GET /pages.xml
  def index
    @pages = group.pages.find(:all, :limit => 20, :order => "updated_at DESC")
    # todo: paginate
  ...
  # GET /pages/1.xml
  def show
    @page = group.pages.find_by_permalink(params[:id] || "home")
  ...
  def diff
    @page = group.pages.find_by_permalink(params[:id])
  ...
  # GET /pages/new.xml
  def new
    @page = group.pages.new(:title => session[:new_title].to_s.gsub("-", " ").capitalize, :permalink => session[:new_title])
  ...
  def create
    ...
    @page.profile = logged_in? ? current_profile : nil
  ...
  def search
    @pages = group.pages.find_with_index("#{params[:query]}")
    respond_to do |format|
      format.html
      format.xml
      format.js
  ...
  # DELETE /pages/1.xml
  def destroy
    @page = group.pages.find_by_permalink(params[:id]).destroy
    respond_to do |format|
  ...
  def require_login
    group.wiki_requires_login_to_post ? login_required : true
  end
