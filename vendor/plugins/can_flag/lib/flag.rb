class Flag < ActiveRecord::Base
  ...
  validates_presence_of :owner_id, :on => :create, 
    ...
    :if => Proc.new { |c| c.flaggable and c.flaggable.owner_id }

  # A user can flag a specific flaggable with a specific flag once
  validates_uniqueness_of :flagger_id, :scope => [:flaggable_id, :flaggable_type]
  ...
  def set_owner_id
    self.owner_id = flaggable.owner_id
  end
